...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/BCZ"
    else
        PARAMS_DIR="$HOME/.bcz"
    fi
fi
...
Failed to fetch the BCZ zkSNARK parameters!
...
This script will fetch the BCZ zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common BCZ zkSNARK parameters. Note that it is
