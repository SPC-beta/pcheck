...
{
    return sporkManager.IsSporkActive(SPORK_7_SAPLING_MAINTENANCE);
}
    ...
    for (const SendCoinsRecipient& rcp : recipients) {
        { // User-entered address / amount:
            ...
            if (rcp.isP2CS) {
                ...
                scriptPubKey = GetScriptForStakeDelegationLOF(*stakerId, *ownerId);
            } else {
...
bool WalletModel::updateAddressBookPurpose(const QString &addressStr, const std::string& purpose)
{
    ...
    if (isStaking)
        return error("Invalid address, cold staking address");
    CKeyID keyID;
...
bool WalletModel::getKeyId(const CTxDestination& address, CKeyID& keyID)
{
    ...
    if (!inKeyID)
        return error("Unable to get KeyID from address");
