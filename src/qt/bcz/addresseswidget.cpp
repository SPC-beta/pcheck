...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto bczAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(bczAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(bczAdd);
