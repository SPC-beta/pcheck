    ...
    coinIcon(new QPushButton())
{
    ...
    ui->btnUri->setSubTitleClassAndText("text-subtitle", tr("Parse a BCZ URI"));
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onBCZSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onBCZSelected(false);});
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent BCZs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res) {
                inform(tr("Error generating address to shield BCZs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent BCZs to shield."));
    }
}
...
void SendWidget::onBCZSelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto bczAdd = Standard::DecodeDestination(address.toStdString(), isStakingAddr);

        if (!Standard::IsValidDestination(bczAdd) || isStakingAddr) {
