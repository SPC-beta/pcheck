...
void DumpMasternodePayments()
{
    ...
    LogPrint(BCLog::MASTERNODE,"mnpayments dump finished  %dms\n", GetTimeMillis() - nStart);
}
...
bool IsBlockPayeeValid(const CBlock& block, const CBlockIndex* pindexPrev)
{
    ...
    if (!g_tiertwo_sync_state.IsSynced()) { //there is no data to use to check anything -- find the longest chain
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txCoinbase, CMutableTransaction& txCoinstake, const CBlockIndex* pindexPrev, bool fProofOfStake) const
{
    ...
    // Starting from BCZ v6.0 masternodes are paid in the coinbase tx
...
bool IsCoinbaseValueValid(const CTransactionRef& tx, CValidationState& _state)
{
    ...
    if (g_tiertwo_sync_state.IsSynced()) {
        const CAmount nCBaseOutAmt = tx->GetValueOut();
        {
            ...
            bool sporkEnforced = sporkManager.IsSporkActive(SPORK_21_MASTERNODE_PAYMENT_ENFORCEMENT);
            ...
            if (!sporkEnforced && nCBaseOutAmt > nMnAmt) {
                return _state.DoS(100, error(strError.c_str()), REJECT_INVALID, "bad-cb-amt-spork21-disabled");
            }
            return true;
        }
    }
