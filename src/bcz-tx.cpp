...
static bool AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help")) {
        ...
        std::string strUsage = PACKAGE_NAME " bcz-tx utility version " + FormatFullVersion() + "\n\n" +
                               "Usage:  bcz-tx [options] <hex-tx> [commands]  Update hex-encoded bcz transaction\n" +
                               "or:     bcz-tx [options] -create [commands]   Create hex-encoded bcz transaction\n" +
                               "\n";
