...
static bool LoadStakeInput(const CBlock& block, std::unique_ptr<CStakeInput>& stake, int nHeight)
{
    ...
    stake = std::unique_ptr<CStakeInput>(CBczStake::NewBczStake(txin, nHeight, block.nTime));
...
bool Stake(const CBlockIndex* pindexPrev, CStakeInput* stakeInput, unsigned int nBits, int64_t& nTimeTx)
{
    ...
    // Get the new time (and verify it's not the same as previous block)
    nTimeTx = GetAdjustedTime();
    if (nTimeTx <= pindexPrev->nTime) {
        LogPrintf("%s : Stake time check failed\n", __func__);
        return false; } //xxxx
...
bool CheckProofOfStake(const CBlock& block, std::string& strError, const CBlockIndex* pindexPrev)
{
    ...
    if (nValueIn < 100 && g_tiertwo_sync_state.IsBlockchainSynced())
    {
        return false;
    }
