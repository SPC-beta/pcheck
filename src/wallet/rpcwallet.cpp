...
UniValue getnewaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "\"bczaddress\"    (string) The new address\n"
...
UniValue sendtoaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 5)
        throw std::runtime_error(
            ...
            "                             The recipient will receive less BCZs than you enter in the amount field.\n"
...
static UniValue CreateColdStakeDelegation(CWallet* const pwallet, const UniValue& params, CTransactionRef& txNew, CReserveKey& reservekey)
{
    ...
    if (!fUseShielded) {
        ...
        CAmount nFeeRequired;
        CScript scriptPubKey = GetScriptForStakeDelegationLOF(*stakeKey, ownerKey);
        ...
        if (sporkManager.IsSporkActive(SPORK_7_SAPLING_MAINTENANCE)) {
...
UniValue delegatestake(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 7)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"staking_addr\"      (string, required) The bcz staking address to delegate.\n"
            "2. \"amount\"            (numeric, required) The amount in BCZ to delegate for staking. eg 100\n"
            "3. \"owner_addr\"        (string, optional) The address corresponding to the key that will be able to spend the stake.\n"
...
UniValue viewshieldtransaction(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
                ...
                "      \"address\" : \"bczaddress\",     (string) The address involved in the transaction\n"
...
UniValue shieldsendmany(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 5)
        throw std::runtime_error(
                ...
                "      \"amount\":amount    (numeric, required) The numeric amount in " + "BCZ" + " is the value\n"
                ...
                "                           Those recipients will receive less BCZ than you enter in their corresponding amount field.\n"
...
UniValue getbalance(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || (request.params.size() > 4 ))
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "amount              (numeric) The total amount in BCZ received for this wallet.\n"
...
UniValue getcoldstakingbalance(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || (request.params.size() != 0))
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "amount              (numeric) The total amount in BCZ received for this wallet in P2CS contracts.\n"
...
UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 6)
        throw std::runtime_error(
            ...
            "                                     the numeric amount in BCZ is the value\n"
            "      ,...\n"
            "    }\n"
...
UniValue addmultisigaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "\"bczaddress\"  (string) A address associated with the keys.\n"
...
UniValue listtransactions(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 6) throw std::runtime_error(
            ...
            "[\n"
            "  {\n"
            "    \"address\":\"bczaddress\",    (string) The address of the transaction.\n"
            "    \"category\":\"category\",      (string) The transaction category (send|receive|orphan|immature|generate).\n"
            "    \"amount\": x.xxx,          (numeric) The amount in BCZ. This is negative for the 'send' category, and positive\n"
...
UniValue listsinceblock(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "{\n"
            "  \"transactions\": [\n"
            "    \"address\":\"bczaddress\",    (string) The address of the transaction. Not present for move transactions (category = move).\n"
...
UniValue getwalletinfo(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"autocombine_threshold\": x.xxx,          (numeric) the current autocombine threshold in BCZ\n"
...
UniValue getstakingstatus(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \blockchain_sync\": true|false,      (boolean) blockchain_sync is synced\n"
            "  \"mnsync\": true|false,              (boolean) whether the required masternode/spork data is synced\n"
            "  \"walletunlocked\": true|false,      (boolean) whether the wallet is unlocked\n"
            "  \"stakeableutxos\": n                (numeric) number of stakeable UTXOs\n"
            ...
            "  \"lastattempt_utxos\": n             (numeric) number of stakeable utxos available during last stake attempt\n"
        ...
        throw JSONRPCError(RPC_IN_WARMUP, "Try again after active chain is loaded");
    {
        ...
        obj.pushKV("blockchain_sync", g_tiertwo_sync_state.IsBlockchainSynced());
        ...
        obj.pushKV("stakeableutxos", (int)vCoins.size());
        ...
        if (ss) {
            ...
            obj.pushKV("lastattempt_utxos", ss->GetLastCoins());
...
UniValue autocombinerewards(const JSONRPCRequest& request)
{
    if (!IsDeprecatedRPCEnabled("autocombinerewards")) {
        if (request.fHelp) {
            throw std::runtime_error("autocombinerewards (Deprecated, will be removed in v6.0. To use this command, start bczd with -deprecatedrpc=autocombinerewards)");
        }
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "autocombinerewards is deprecated and will be removed in v6.0. To use this command, start bczd with -deprecatedrpc=autocombinerewards");
    }
...
UniValue setautocombinethreshold(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.empty() || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "  \"threshold\": n.nnn,         (numeric) auto-combine threshold in BCZ\n"
...
UniValue getautocombinethreshold(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || !request.params.empty())
        throw std::runtime_error(
            ...
            "{\n"
            "  \"enabled\": true|false,        (boolean) true if auto-combine is enabled, otherwise false\n"
            "  \"threshold\": n.nnn            (numeric) the auto-combine threshold amount in BCZ\n"
            "}\n"
...
{ //  category              name                        actor (function)           okSafe argNames
  //  --------------------- ------------------------    -----------------------    ------ --------
    ...
    { "wallet",             "whitelist",                &delegatoradd,             true,  {"address","label"} },
