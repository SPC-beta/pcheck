                                      ...
                                      std::string& strError)
{
    ...
    if (txOut.nValue != consensus.nMNCollateralAmt) {
        strError = strprintf("Invalid collateral tx value, must be %s BCZ", FormatMoney(Params().GetConsensus().nMNCollateralAmt));
...
{
    const bool fIncludeColdStaking = !sporkManager.IsSporkActive(SPORK_26_COLDSTAKING_MAINTENANCE) &&
...
bool CWallet::CreateCoinstakeOuts(const CBczStake& stakeInput, std::vector<CTxOut>& vout, CAmount nTotal) const
{
        ...
        bool stopOnNewBlock) const
{
    ...
    for (auto it = availableCoins->begin(); it != availableCoins->end();) {
        ...
        CBczStake stakeInput(it->tx->tx->vout[it->i],
                             outPoint,
                             it->pindex);
