                              ...
                              std::vector<COutPoint>& coins_to_uncache) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (sporkManager.IsSporkActive(SPORK_7_SAPLING_MAINTENANCE) && tx.IsShieldedTx())
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    bool fSaplingMaintenance =  (block.nTime > sporkManager.GetSporkValue(SPORK_7_SAPLING_MAINTENANCE));
    ...
    if (isPoSBlock && isV6UpgradeEnforced && !IsCoinbaseValueValid(block.vtx[0], state)) {
        // pass the state returned by the function above
        return false;
    }
