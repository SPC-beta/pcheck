...
// Copyright (c) 2015-2022 The BCZ developers
...
void Misbehaving(NodeId pnode, int howmuch, const std::string& message) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (state->nMisbehavior >= banscore && state->nMisbehavior - howmuch < banscore) {
        ...
        if (sporkManager.IsSporkActive(SPORK_4_BAN_MISBEHAVING)){
                                      ...
                                      CNetMsgMaker& msgMaker)
{
    ...
    if (inv.type == MSG_QUORUM_FINAL_COMMITMENT) {
        ...
        if (sporkManager.IsSporkActive(SPORK_9_LLMQ_DKG_MAINTENANCE)) return false;
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived, CConnman* connman, std::atomic<bool>& interruptMsgProc)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        bool fMissingSporks = !pSporkDB->SporkExists(SPORK_14_NEW_PROTOCOL_ENFORCEMENT) ||
                              ...
                              !pSporkDB->SporkExists(SPORK_7_SAPLING_MAINTENANCE);
