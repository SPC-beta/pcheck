...
fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", BCZ_PID_FILENAME));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf("Specify pid file (default: %s)", BCZ_PID_FILENAME));
#endif
...
static void LoadSaplingParams()
{
    ...
    //fs::path ZC_GetBaseParamsDir();
    try {
        initZKSNARKS();
    } catch (std::runtime_error &e) {
        uiInterface.ThreadSafeMessageBox(strprintf(
                _("Cannot find the Sapling parameter files!\n"
                  "Please copy 'sapling-output.params' and sapling-spend.params'\n"
                  "into BCZ datafolder:" "%s,\n"
                  "or run 'sapling-fetch-params' or './util/fetch-params.sh' and then restart."),
                GetDefaultDataDir()),
...
bool AppInitMain()
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", BCZ_CONF_FILENAME)).string());
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile because if BCZ is started in the future "
                  "from a different location. It will be unable to locate the current data files. There could "
                  "also be data loss if BCZ is started while in a temporary directory.\n",
