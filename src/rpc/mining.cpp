...
UniValue generateBlocks(const Consensus::Params& consensus,
                        ...
                        int nHeight,
                        int nHeightEnd,
                        CScript* coinbaseScript, uint64_t nMaxTries, bool keepScript)
{
    if (fPoS) {
    ...
    while (nHeight < nHeightEnd && !ShutdownRequested()) {
        ...
        std::unique_ptr<CBlockTemplate> pblocktemplate(BlockAssembler(Params(), DEFAULT_PRINTPRIORITY).CreateNewBlock(CScript(), pwallet, true, &availableCoins));
    ...
    else
    {
        ...
        while (nHeight < nHeightEnd && !ShutdownRequested())
        {
            ...
            if (!ProcessNewBlock(shared_pblock, nullptr))
                throw JSONRPCError(RPC_INTERNAL_ERROR, "ProcessNewBlock, block not accepted");
            ++nHeight;
            blockHashes.push_back(pblock->GetHash().GetHex());
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    result.pushKV("enforce_masternode_payments", sporkManager.IsSporkActive(SPORK_21_MASTERNODE_PAYMENT_ENFORCEMENT));
