...
static bool ProcessBlockFound(const std::shared_ptr<const CBlock>& pblock, CWallet& wallet, std::unique_ptr<CReserveKey>& reservekey)
{
    LogPrintf("%s\n", pblock->ToString());
...
void BitcoinMiner(CWallet* pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (pwallet->pStakerStatus && pwallet->pStakerStatus->GetLastHash() == pindexPrev->GetBlockHash()) {
                MilliSleep(2000);
                continue;
            }
